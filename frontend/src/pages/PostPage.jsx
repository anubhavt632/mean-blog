...
export default function PostPage() {
    ...
    useEffect(() => {
        try {
            const fetchRecentPosts = async () => {
                const res = await fetch(`/backend/post/getposts?limit=2`);
                const data = await res.json();
                if (res.ok) {
    ...
    if (error) return (
        <div className="flex justify-center items-center min-h-screen">
            <p>Error occurred while loading post.</p>
        </div>
    );
    ...
    return (
        <main className="p-3 flex flex-col max-w-6xl mx-auto min-h-screen">
            ...
            <img src={post && post.image} alt={post && post.title} className="mt-19 p-3 max-h-[600px] w-full object-cover"/>
